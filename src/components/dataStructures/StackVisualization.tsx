...
export const StackVisualization: React.FC<StackVisualizationProps> = ({
  ...
  const renderStack = () => {
    ...
    stack.forEach((value, index) => {
      const y = stackBottomY - (index + 1) * cellHeight;
    ...
    // Draw empty slots
    for (let i = stack.length; i < maxSize; i++) {
      const y = stackBottomY - (i + 1) * cellHeight;
    ...
    // Stack operations indicators
    svg.append('text')
      .attr('x', stackX - 60)
      .attr('y', stackBottomY - maxSize * cellHeight - 20)
      .attr('text-anchor', 'middle')
      .attr('fill', '#10b981')
      .attr('font-size', '12px')
      .attr('font-weight', 'bold')
  ...
  const handlePush = () => {
    ...
    setLastOperation(`Pushed ${value} onto the stack`);
  ...
  const handlePop = () => {
    ...
    setLastOperation(`Popped ${poppedValue} from the stack`);
